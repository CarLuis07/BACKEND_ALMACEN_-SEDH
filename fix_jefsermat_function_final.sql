-- FIX: Remover validación que bloquea re-aprobaciones en responder_requisicion_jefe_materiales
-- La validación "ya fue respondida" impide que se corrijan errores o se vuelva a aprobar

DROP FUNCTION IF EXISTS requisiciones.responder_requisicion_jefe_materiales(UUID, TEXT, TEXT, TEXT, JSONB) CASCADE;
...
BEGIN
    ...
    -- REMOVIDO: Validación que bloqueaba re-aprobaciones
    -- Ahora permitimos múltiples intentos (solo se insertará si no existe)
